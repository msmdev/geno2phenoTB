...
============
    ...
    geno2phenoTB is a tool to predict resistance of *Mycobacterium tuberculosis* against antibiotics
    using WGS data.
...
mutation catalog (see [`Grobbel21`_] and [`Finci22`_]) of the Research Center Borstel - Leibniz
...
Both require the path to a directory with 2 fastq files as input.
...
Python interface
----------------
Import `geno2phenotb` and use the `predict` function of the `predict` submodule:
...
.. code-block:: console
    ...
    feature_evaluation : pd.DataFrame
        ...
        genotypes) plus an assessment of the relevance of each feature.
